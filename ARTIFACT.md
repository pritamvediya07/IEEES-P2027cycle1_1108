...
| Badge | Justification |
|---|---|
| **Artifacts Available** | Archived on Zenodo with a DOI after evaluation, before the camera-ready deadline (14 Oct 2026); the paper (IEEE_S_P.pdf) links this GitHub repository, and the camera-ready will also cite the DOI. The GitHub repository (https://github.com/pritamvediya07/IEEES-P2027cycle1_1108) is the evaluation copy. Contents: complete PALA source, the Docker UERANSIM testbed, the srsRAN testbed configuration and experiment scripts, 1,418 stored UERANSIM result files (JSON/JSONL under `final_experiments/`), all srsRAN results with per-trial LLM traces, and the figure, table, and verification scripts |
