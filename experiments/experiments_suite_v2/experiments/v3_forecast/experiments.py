...
def run_e3_1():
    ...
    # Fetch reference series
    values, timestamps = read_analytics("upf_metrics", "memory_util_pct", V3_REFERENCE_SERIES_LEN)
    print(f"  Reference series: {len(values)} samples collected")
    ...
    for r in all_results:
        row = [r["N"], r["width"]["median"], r["width"]["iqr_25"], r["width"]["iqr_75"],
               r["mae"]["median"], r["rmse"]["median"]]
        if isinstance(r["r2_test"], dict) and r["r2_test"].get("median") is not None:
            row.append(r["r2_test"]["median"])
        else:
            row.append("")
        row.append(r["r2_train"]["median"])
        rows.append(row)
    save_csv(rows,
