...
# experiment-level config.py file.
...
def run_iperf3(duration=30, interface="uesimtun0", udp=True, bandwidth_mbps=None, server="10.45.0.1"):
    ...
    Uses --bind <interface_ip> to route through the UE tunnel without
    requiring root (--bind-dev is unsupported on iperf3 < 3.10).
    bandwidth_mbps: offered load; if None, defaults to max.
    """
    iface_ip = _get_interface_ip(interface)
    cmd = ["iperf3", "-c", server, "-t", str(duration), "-J"]
    if iface_ip:
        cmd.extend(["--bind", iface_ip])
    if udp:
        cmd.append("-u")
        if bandwidth_mbps:
    ...
    try:
        ...
        if not data.get("end"):
            print(f"  [WARN] iperf3 returned no results (connected={data.get('start',{}).get('connected',[])})")
            return None
        if udp:
